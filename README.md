...
Omniscitus keeps a footprint of every file it touches outside `.omniscitus/` (mainly the `CLAUDE.md` block and any project-specific markers). Clean removal is a two-step dance:

```
# 1. Surgical cleanup FIRST — before uninstalling the plugin
/omniscitus-uninstall

# 2. Then remove the plugin
/plugin uninstall omniscitus
```

`/omniscitus-uninstall` reads `.omniscitus/migrate/anchor.yaml` (recorded at migrate time), removes appended sections using their saved markers, restores modified files via `git checkout {anchor-sha} -- <path>`, and finally deletes `.omniscitus/`. Because this relies on git, the migrate skill refuses to run in a non-git project without explicit user opt-in.

If you already ran `/plugin uninstall` first by mistake, follow the manual fallback in `.omniscitus/README.md` (auto-generated by migrate).
