 ...
 * file, start, end, unit_count, generated_at, domains.
 *
 * Handles both YAML list styles the _index.yaml can end up in:
 *   (a) indented, quoted (what weekly-backfill.cjs writes):
 *         weekly_summaries:
 *           - week: "2026-W14"
 *             file: "..."
 *   (b) block, unquoted (what js-yaml/PyYAML default dumps produce when
 *       some other tool round-trips the file):
 *         weekly_summaries:
 *         - week: 2026-W14
 *           file: ...
 * `domains:` similarly accepts inline `[a, b]` or block-list form.
 */
function parseWeeklySummariesYaml(text) {
  ...
  for (var i = 0; i < lines.length; i++) {
    ...
    // A new top-level key (non-indented, non-blank, not a list item) ends
    // the section. `-` at column 0 is a block-style list item and must NOT
    // terminate the section.
    if (line.length > 0 && line[0] !== ' ' && line[0] !== '-' &&
        line.trim().length > 0 && !/^weekly_summaries/.test(line)) {
      if (current) { summaries.push(current); current = null; }
      inSection = false;
      inDomainsBlock = false;
      continue;
    }
    ...
    if (current) {
      // Block-form `domains:\n  - a\n  - b` — collect list items until a
      // non-list line appears.
      if (inDomainsBlock) {
        var domainItem = line.match(/^\s+-\s+["']?([^"'\s#]+)["']?\s*(#.*)?$/);
        if (domainItem) {
          current.domains.push(domainItem[1]);
          continue;
        }
        inDomainsBlock = false;
        // fall through — this line may be a new prop
      }
      ...
      var domainInline = line.match(/^\s+domains:\s*\[(.*)\]\s*$/);
      if (domainInline) {
        current.domains = domainInline[1].split(',')
          .map(function (s) { return s.trim().replace(/^["']|["']$/g, ''); })
          .filter(function (s) { return s.length > 0; });
        continue;
      }
      if (/^\s+domains:\s*$/.test(line)) {
        current.domains = [];
        inDomainsBlock = true;
        continue;
      }
